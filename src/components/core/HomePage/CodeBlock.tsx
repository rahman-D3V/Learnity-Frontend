...
const code = `async function getUsers() {
...
function tokenizeLine(line: string): Token[] {
  ...
  parts.forEach((part) => {
    if (jsKeywords.includes(part)) {
      tokens.push({ text: part, color: "#c678dd" }); // purple
    } else if (/^".*"$/.test(part)) {
      tokens.push({ text: part, color: "#98c379" }); // green string
    } else if (/^\d+$/.test(part)) {
      tokens.push({ text: part, color: "#d19a66" }); // number
    } else if (part === "fetch") {
      tokens.push({ text: part, color: "#61afef" }); // function
    } else {
      tokens.push({ text: part, color: "#abb2bf" }); // default
    }
  });
...
export default function CodeBlock() {
  ...
  return (
    <div className=" relative flex items-center  justify-center bg-[#070b12] font-mono">
      <div className="absolute inset-5 bg-gradient-to-r overflow-hidden from-orange-500 to-black blur-3xl z-10 opacity-25"></div>
